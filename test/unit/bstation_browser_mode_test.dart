...
import 'package:nobarin/features/browser/presentation/bstation_browser_sheet.dart';
...
void main() {
  ...
  group('BstationBrowserMode Enum Tests', () {
    test('enum has all required modes', () {
      expect(BstationBrowserMode.values, contains(BstationBrowserMode.general));
      expect(BstationBrowserMode.values, contains(BstationBrowserMode.queueOnly));
      expect(BstationBrowserMode.values, contains(BstationBrowserMode.createRoom));
      expect(BstationBrowserMode.values, contains(BstationBrowserMode.watchNow));
    });
  });

  group('UnifiedPlayerController Bstation URL Detection Tests', () {
    test('detects bilibili.tv URLs properly', () {
      final detected = UnifiedPlayerController.detectMediaFromUrl(
        'https://www.bilibili.tv/id/video/2048573920',
      );
      expect(detected, isNotNull);
      expect(detected!.mediaType, equals('bstation'));
      expect(detected.isBstation, isTrue);
      expect(detected.isYouTube, isFalse);
      expect(detected.mediaId, equals('2048573920'));
    });

    test('detects bilibili.tv play/anime URLs properly', () {
      final detected = UnifiedPlayerController.detectMediaFromUrl(
        'https://www.bilibili.tv/id/play/1048837',
      );
      expect(detected, isNotNull);
      expect(detected!.mediaType, equals('bstation'));
      expect(detected.mediaId, equals('1048837'));
    });

    test('detects bilibili.com BV URLs properly', () {
      final detected = UnifiedPlayerController.detectMediaFromUrl(
        'https://www.bilibili.com/video/BV1xx411c7mD',
      );
      expect(detected, isNotNull);
      expect(detected!.mediaType, equals('bstation'));
      expect(detected.mediaId, equals('BV1xx411c7mD'));
    });

    test('detects b23.tv short URLs properly', () {
      final detected = UnifiedPlayerController.detectMediaFromUrl(
  ...
  group('BstationBrowserSheet Widget & Mode Tests', () {
    ...
    testWidgets('renders fallback UI with createRoom mode', (tester) async {
      ...
      // Verify mode title in top bar
      expect(find.text('Pilih Video Bstation'), findsOneWidget);
      ...
      expect(selectedUrl, equals('https://www.bilibili.tv/id/video/2048573920'));
    });
    ...
    testWidgets('renders fallback UI with watchNow mode', (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: BstationBrowserSheet(
              mode: BstationBrowserMode.watchNow,
              syncController: syncController,
            ),
          ),
        ),
    ...
    testWidgets('queueOnly mode adds to queue on fallback submit', (tester) async {
      ...
      expect(queueController.items.first.mediaUrl,
          equals('https://www.bilibili.tv/id/video/2048573920'));
      expect(queueController.items.first.mediaType, equals('bstation'));
      expect(queueController.items.first.isBstation, isTrue);
    });
  });
}
